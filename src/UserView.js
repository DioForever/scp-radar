...
let ScpImage = `
<svg width="135" height="135" version="1.1" viewBox="0 0 135 135" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
...
class Mark {
  constructor(id, position, title, type, price) {
...
const postMark = () => {
  ...
  useEffect(() => {
    if (id === "") {
      ...
      else setUserType("SCP Foundation");
      console.log("Stored id: " + storedId); // Logging the stored ID
      if (!storedId) {
        const newUuid = uuidv4(); // Corrected function call to uuidv4()
        setId(newUuid);
        Cookies.set('id', newUuid, { expires: 7 }); // Corrected to use newUuid
        console.log("New id generated: " + newUuid); // Logging the new ID
      } else {
        setId(storedId);
      }
    }
    ...
    const interval = setInterval(() => {
      ...
      if (currentTime.getTime() >= timer.getTime()-10 && userLocation != null && id != "" && userLocation.lat != null) {
      ...
      <div className="user-section">
        ...
        <div>
          ...
          <input
            ...
            type="text"
            value={userType}
            onChange={(e) => {setUserType(e.target.value); Cookies.set('type', e.target.value, { expires: 7 });}}
          />
        </div>
      </div>
